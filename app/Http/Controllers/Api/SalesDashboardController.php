...
class SalesDashboardController extends Controller
{
    ...
    public function getSalesSummary(Request $request)
    {
        try {
            ...
            // Use completed_at for completed sales, created_at for pending
            ...
            $currency = 'KES';
            if ($firstSale) {
                if (is_object($firstSale->currency)) {
                    $currency = $firstSale->currency->code;
                } elseif (!empty($firstSale->currency)) {
    ...
    public function getSalesComparison(Request $request)
    {
        try {
            ...
            $currentQuery->where(function ($q) use ($dateFrom, $dateTo) {
                $q->whereDate('completed_at', '>=', $dateFrom)
                    ->whereDate('completed_at', '<=', $dateTo)
                    ->orWhere(function ($sub) use ($dateFrom, $dateTo) {
                        $sub->whereNull('completed_at')
            ...
            $previousQuery->where(function ($q) use ($previousStart, $previousEnd) {
                $q->whereDate('completed_at', '>=', $previousStart->toDateString())
                    ->whereDate('completed_at', '<=', $previousEnd->toDateString())
                    ->orWhere(function ($sub) use ($previousStart, $previousEnd) {
                        $sub->whereNull('completed_at')
                            ->whereDate('created_at', '>=', $previousStart->toDateString())
                            ->whereDate('created_at', '<=', $previousEnd->toDateString());
                    });
            });
    ...
    public function getTopSellingProducts(Request $request)
    {
        try {
            ...
            $query->where(function ($q) use ($dateFrom, $dateTo) {
                $q->whereDate('sales.completed_at', '>=', $dateFrom)
                    ->whereDate('sales.completed_at', '<=', $dateTo)
                    ->orWhere(function ($sub) use ($dateFrom, $dateTo) {
                        $sub->whereNull('sales.completed_at')
        ...
        try {
            ...
            $query = Sale::select(
                ...
                ->where(function ($q) use ($startDate, $endDate) {
                    $q->whereDate('completed_at', '>=', $startDate->toDateString())
    ...
    public function getPeakSalesHours(Request $request)
    {
        try {
            ...
            for ($hour = 0; $hour < 24; $hour++) {
                ...
                $allHours[] = [
                    'hour' => $hour,
                    'time_label' => \Carbon\Carbon::createFromTime($hour, 0)->format('g A'),
                    'transaction_count' => $existingData ? $existingData->transaction_count : 0,
                    'total_sales' => $existingData ? (float) $existingData->total_sales : 0,
                    'average_order_value' => $existingData ? round($existingData->average_order_value, 2) : 0
                ];
    ...
    public function getProductSummary(Request $request)
    {
        try {
            ...
            $previousDateFrom = \Carbon\Carbon::parse($dateFrom)->subDays($daysDiff)->toDateString();
            $previousDateTo = \Carbon\Carbon::parse($dateFrom)->subDay()->toDateString();
        ...
        try {
            ...
            $productData = $slowMovingProducts->map(function ($product) use ($days) {
                $daysSinceLastSale = $product->last_sale_date
                    ? \Carbon\Carbon::parse($product->last_sale_date)->diffInDays(now())
                    : null;
    ...
    public function getCreditSalesSummary(Request $request)
    {
        try {
            ...
            $overdueQuery = Sale::where('business_id', $businessId)
                ->where('is_credit_sale', true)
                ->where('payment_status', '!=', 'paid')
                ->where('credit_due_date', '<', now()->startOfDay());
    ...
    /**
     * Missing Endpoint: Stock Turnover Rate
     */
    public function getStockTurnoverRate(Request $request)
    {
        try {
            $businessId = $request->user()->business_id;
            $branchId = $request->input('branch_id', $request->user()->primary_branch_id);

            // Simple calculation: Total Quantity Sold / Current Stock Quantity
            $totalSold = SaleItem::join('sales', 'sale_items.sale_id', '=', 'sales.id')
                ->where('sales.business_id', $businessId)
                ->when($branchId, fn($q) => $q->where('sales.branch_id', $branchId))
                ->whereIn('sales.status', ['completed', 'pending']) // Include pending
                ->sum('sale_items.quantity');

            $currentStock = Stock::whereHas('branch', fn($q) => $q->where('business_id', $businessId))
                ->when($branchId, fn($q) => $q->where('branch_id', $branchId))
                ->sum('quantity');

            $turnoverRate = $currentStock > 0 ? ($totalSold / $currentStock) : 0;

            return successResponse('Stock turnover rate retrieved', [
                'turnover_rate' => round($turnoverRate, 2),
                'total_sold' => (float) $totalSold,
                'current_stock_quantity' => (float) $currentStock
            ]);
        } catch (\Exception $e) {
            Log::error('Stock turnover error', ['error' => $e->getMessage()]);
            return queryErrorResponse('Failed to get stock turnover rate', $e->getMessage());
        }
    }

    /**
     * Missing Endpoint: Sales By Branch
     */
    public function getSalesByBranch(Request $request)
    {
        try {
            ...
            return successResponse('Sales by branch retrieved', ['branches' => $data]);
        } catch (\Exception $e) {
            Log::error('Sales by branch error', ['error' => $e->getMessage()]);
            return queryErrorResponse('Failed to get sales by branch', $e->getMessage());
        }
    }

    /**
     * Missing Endpoint: Average Transaction Value
     */
    public function getAverageTransactionValue(Request $request)
    {
        try {
            $businessId = $request->user()->business_id;
            $branchId = $request->input('branch_id', $request->user()->primary_branch_id);
            ...
            return successResponse('ATV retrieved', [
                'average_transaction_value' => round($average ?? 0, 2),
                'transaction_count' => $count
            ]);
        } catch (\Exception $e) {
            Log::error('ATV error', ['error' => $e->getMessage()]);
            return queryErrorResponse('Failed to get ATV', $e->getMessage());
        }
    }

    /**
     * Missing Endpoint: Customer Purchase Frequency
     */
    public function getCustomerPurchaseFrequency(Request $request)
    {
        try {
            $businessId = $request->user()->business_id;
            $branchId = $request->input('branch_id', $request->user()->primary_branch_id);

            // Total Sales / Total Distinct Customers
            $query = Sale::where('business_id', $businessId)
                ->when($branchId, fn($q) => $q->where('branch_id', $branchId))
                ->whereIn('status', ['completed', 'pending']);
            ...
            $frequency = $uniqueCustomers > 0 ? ($totalSales / $uniqueCustomers) : 0;

            return successResponse('Purchase frequency retrieved', [
                'purchase_frequency' => round($frequency, 2),
                'total_transactions' => $totalSales,
                'unique_customers' => $uniqueCustomers
            ]);
        } catch (\Exception $e) {
            Log::error('Purchase frequency error', ['error' => $e->getMessage()]);
            return queryErrorResponse('Failed to get purchase frequency', $e->getMessage());
        }
    }

    /**
     * Missing Endpoint: Product Margin Analysis
     */
    public function getProductMarginAnalysis(Request $request)
    {
        try {
            $businessId = $request->user()->business_id;
            $branchId = $request->input('branch_id', $request->user()->primary_branch_id);
            ...
            return successResponse('Margin analysis retrieved', ['products' => $data]);
        } catch (\Exception $e) {
            Log::error('Margin analysis error', ['error' => $e->getMessage()]);
            return queryErrorResponse('Failed to get margin analysis', $e->getMessage());
        }
    }
}
